...
The formalization comprises 1,592 lines of \Lean~4, proves 53
...
\begin{enumerate}[nosep, label=(\arabic*)]
  ...
  \item A three-way decomposition of all 53 formalized results
...
\noindent
The formalization comprises 1,592 lines of \Lean~4 across two
files.\footnote{%
...
one atom---as do the separation witnesses of
Theorem~\ref{thm:hierarchy}, which name specific atoms of the
concrete type \texttt{TwoFacts}.  The expressibility collapse
...
$\mathtt{elementary}(\mathit{rain})$ and
$\mathtt{elementary}(\mathit{snow})$ are $\formEq$ (via the
swap permutation) but not $\semEq$: they differ in truth value
at the world where it rains and does not snow
(\texttt{formEq\_not\_\allowbreak{}semEq\_\allowbreak{}witness}).  Conversely,
$\lnot\lnot(\mathtt{elementary}\;s)$ is $\semEq$ to
$\mathtt{elementary}\;s$ (double negation) but not $\formEq$,
since renaming preserves tree depth
...
\emph{(3) The intersection.}  Commuted conjunctions
$\mathit{rain} \wedge \mathit{snow}$ and
$\mathit{snow} \wedge \mathit{rain}$ share their logical form
(swap permutation) and their truth conditions (commutativity of
$\wedge$) yet are not structurally equal
...
But this relation is far weaker than it looks: with classical
choice, \emph{any} two nontrivial propositions are related by
some such~$f$, so it cannot serve as an intermediate notion
between $\formEq$ and $\semEq$.  We record it as a lemma, not
as a rung of a hierarchy.
\end{remark}
...
First, formalizing the $N$-operator and proving the
finite-domain quantifier connection (that $\forall x.\, P(x)$
reduces to an $N$-application over a \texttt{Fintype} domain)
would complete the truth-functional picture for the finite
case and sharpen the boundary with the infinite case.
...
\Lean~4.  The formalization proves 53 formally verified results
across two files, decomposes the
...
Proof files:
\begin{itemize}[nosep]
  \item \texttt{proofs/TractatusOntology.lean} --- 1,308 lines, 44 declarations
...
\centering
\footnotesize
\caption{Complete listing of all 53 theorem/lemma declarations
...
\midrule
\multicolumn{5}{l}{\emph{TractatusOntology.lean} (44 declarations)} \\
\midrule
...
32 & \texttt{formEq\_not\_structEq\_witness}             & 4.0141  & Ont. & L \\
33 & \texttt{semEq\_not\_formEq\_witness}                & 4.0141  & Ont. & L \\
34 & \texttt{formEq\_not\_semEq\_witness}                & 4.0141  & Ont. & L \\
35 & \texttt{formEq\_semEq\_not\_structEq\_witness}      & 4.0141  & Ont. & L \\
36 & \texttt{structEq\_ne\_semEq}                        & 4.0141  & Ont. & L \\
37 & \texttt{world\_constant\_taut\_or\_contra}          & 4.46    & Ont. & L \\
38 & \texttt{saying\_showing\_triviality}                & 4.12    & Ont. & L \\
39 & \texttt{contingent\_propositions\_vary}             & 4.46    & Ont. & L \\
40 & \texttt{nontrivial\_of\_not\_taut\_not\_contra}     & 4.46    & Ont. & L \\
41 & \texttt{nontrivial\_iff\_not\_taut\_not\_contra}    & 4.46    & Ont. & L \\
42 & \texttt{proposition\_seven}                         & 7       & Ont. & L \\
43 & \texttt{nontrivial\_expressibility\_requires\_world\_dependence}
                                                         & 4.12    & Ont. & L \\
44 & \texttt{nontrivial\_cannot\_express\_world\_independent}
                                                         & 4.12    & Ont. & L \\
\midrule
\multicolumn{5}{l}{\emph{TractatusQuantifiers.lean} (9 declarations)} \\
\midrule
45 & \texttt{truth\_functional\_compositionality\_fo}    & 5       & Quant. & I \\
46 & \texttt{forall\_exists\_duality}                    & 5.52    & Quant. & I \\
47 & \texttt{forall\_distrib\_conj}                      & 5.52    & Quant. & I \\
48 & \texttt{exists\_distrib\_disj\_reverse}             & 5.52    & Quant. & I \\
49 & \texttt{lift\_eval}                                 & ---     & Quant. & I \\
50 & \texttt{forall\_vacuous}                            & ---     & Quant. & I \\
51 & \texttt{double\_negation\_fo}                       & 5.101   & Quant. & I \\
52 & \texttt{not\_exists\_forall\_not}                   & 5.52    & Quant. & I \\
53 & \texttt{fo\_bivalence}                              & 4.023   & Quant. & I \\
\midrule
