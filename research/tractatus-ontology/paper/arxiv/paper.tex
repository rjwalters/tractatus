...
% It uses the ASL author document class asl.cls (v1.3, 2002/06/07;
% CUP/ASL Author Resources bundle, dated 2023/12, obtained from
% https://aslonline.org/journals/author-resources/), the class the
% ASL Typesetting Office distributes for author submissions to the
% Journal / Bulletin / Review of Symbolic Logic.  RSL formatting
% conventions (theorem environments, ASL bibliography style, MSC
% 2020 codes, keywords) are followed via the class front-matter
% commands.  The CUP production class rsl.cls, shipped in the same
% bundle, is a typesetter-only class: it \input's CUP-internal files
% (rslstandard.tex, rsl-queryform.tex) that are not distributed to
% authors and cannot compile outside CUP, so it is not usable at
% author-submission stage.  See the git commit / PR for provenance.
%
% Modern-TeX compatibility: the 2002-era asl.cls redefines
% \negmedspace / \negthickspace with \newcommand, which now clash
% with the LaTeX kernel's own definitions.  Undefining them before
% \documentclass lets asl.cls install its versions cleanly, leaving
% the vendored class byte-identical to the ASL distribution.
\makeatletter
\let\negmedspace\@undefined
\let\negthickspace\@undefined
\makeatother
\documentclass[jsl,bibother]{asl}

% =====================================================================
% arXiv PREPRINT VARIANT (issue #33)
% ---------------------------------------------------------------------
% This file is byte-identical to the RSL camera-ready
% (tractatus-ontology.11/paper.tex) EXCEPT for this block and a single
% title-page headnote (\thanks on \title).  It exists so the paper can
% be posted to arXiv as a preprint of a manuscript under review WITHOUT
% carrying the Review of Symbolic Logic / ASL journal masthead or the
% ASL copyright line -- neither of which belongs on a preprint that the
% journal has not yet accepted or typeset.
%
% The [jsl] class option turns on two pieces of journal branding:
%   * \LogoOn      -- the first-page masthead logo box, which prints the
%                     journal name (overridden to "Review of Symbolic
%                     Logic" below) plus a Volume/Number/Year line;
%   * \CopyRightOn -- the "(c) year, Association for Symbolic Logic" +
%                     ISSN copyright block in the first-page margin.
% We switch BOTH off here.  This touches only toggles that asl.cls
% itself exposes (\LogoOnfalse / \CopyRightfalse); the vendored asl.cls
% is left byte-identical to the ASL distribution.  The running heads on
% pp. 2ff use the author/title marks (\leftmark/\rightmark), not the
% journal name, so no further change is needed for them.  The result is
% a neutral ASL-journal article layout with no journal identification.
%
% These MUST be set in the preamble (not deferred): asl.cls registers
% its own \AtBeginDocument{\ifLogoOn\@setlogo\fi} at class-load time, so
% \ifLogoOn must already be false when that hook fires.
\makeatletter
\LogoOnfalse
\CopyRightfalse
\makeatother
...
% asl.cls ships venue options for the Journal (jsl), Bulletin (bsl)
% and conference proceedings, but no dedicated Review-of-Symbolic-Logic
% option (the RSL production class rsl.cls is typesetter-only; see the
% note above).  We therefore build on the jsl venue -- which gives the
% correct ASL journal-article layout -- and override the masthead name
% so the running head reads ``Review of Symbolic Logic'' rather than
% ``Journal of Symbolic Logic''.  Volume/number/copyright are placeholders
% that the ASL Typesetting Office resets at production time.
\makeatletter
\def\journalname{The Review of Symbolic Logic}
\def\shortjournalname{The Review of Symbolic Logic}
\def\longjournalname{The \hfil Review \hfil of \hfil Symbolic \hfil Logic}
% asl.cls only knows the 1991 and 2000 MSC schedules; set the 2020 label
% that RSL requires.  \subjclass{...} in the front matter supplies the codes.
\def\subjclassname{\textup{2020} Mathematics Subject Classification}
\makeatother

% asl.cls provides amsmath, amssymb, amsthm, and the AMS front matter.
% Color is NOT permitted in ASL/RSL submissions, so xcolor is omitted
% and all listings / diagrams below are rendered in black and grey.
\usepackage{booktabs}
\usepackage{graphicx}
\usepackage{enumitem}
...
\usepackage{listings}
\usepackage{tikz}
\usetikzlibrary{positioning, fit, calc, arrows.meta, patterns, patterns.meta}
% hidelinks: no coloured link borders or text -- ASL/RSL is printed in
% black only, and hyperref's default coloured/boxed links are not
% permitted.  Navigation still works; the links are simply invisible.
...
% Lean code formatting (colour-free: ASL/RSL submissions are printed
% in black only, so keywords/comments/strings are distinguished by
% weight and shape rather than by colour).
...
% ASL's text column is narrower than the article default, and several
% long, unbreakable \texttt Lean identifiers (dotted file names such as
% TractatusExpressibility.lean and snake_case declaration names) cannot
% hyphenate.  Two content-neutral measures address this: \emergencystretch
% lets TeX absorb small excesses by stretching interword space on the
% affected lines, and \allowbreak markers (inserted only at `.'/`_'
% boundaries inside the longest identifiers below) supply zero-width
% break opportunities so the names can wrap.  Neither alters printed text.
\emergencystretch=3em
% Hyphenation hints for compound / prefixed terms that TeX will not
% break on its own; these only add legal break points and never change
% the printed spelling.  Needed because the ASL theorem font sets some
% statement lines wider than the article default.
\hyphenation{non-trivial non-triviality world-independent
  truth-functional counter-example de-duplicated}

% --- ASL/RSL front matter ---
% ASL house style sets only proper names in upper case in titles; the
% class up-cases the whole title itself, so entries are given in
% ordinary case here.  The short title (optional argument) is used in
% the running head.
\title[What Lean Cannot Say]{What \Lean{} Cannot Say:
A Machine-Checked Analysis of Wittgenstein's \emph{Tractatus}}
% arXiv preprint headnote.  asl.cls's \title does not emit a
% title-attached \thanks footnote, so the "under review" notice is
% carried as a footnote on the author name instead (\footnote works in
% the ASL front matter -- cf. the GitHub-artifact footnote in the
% abstract below).
\author{Robb Walters%
\footnote{Preprint. Manuscript under review. This is not the journal's
  copy of record; it is posted to arXiv under a non-exclusive licence
  to establish priority ahead of journal submission.}}
\revauthor{Walters, Robb}
\address{Integrated Plasmonics Corporation\\
  San Francisco, CA, USA}
\email{robb@integratedplasmonics.com}
\keywords{Wittgenstein, \emph{Tractatus Logico-Philosophicus},
  proof assistant, Lean~4, formal verification, truth-functional
  semantics, saying/showing distinction, expressibility, $N$-operator}
\subjclass{Primary 03B70; Secondary 03A05, 03B35, 03B05}
...
assistant.  Parameterizing over world models separates the
\emph{Tractatus} into invariants, assumptions, and limits:
truth-functional compositionality, bivalence, and functional
completeness are structural invariants holding in every model,
whereas the independence of atomic facts is a model-dependent
assumption---it characterizes a ``free'' model and fails in
constrained ones.  An \texttt{expresses} relation linking
propositions to meta-level predicates lets us prove that any
proposition expressing a world-independent property is a
tautology or contradiction, so nontrivial propositions cannot
express world-independent truths---the saying/showing distinction
as a theorem.  Distinguishing three equivalences---syntactic
identity, logical-form equivalence up to permutation, and
semantic equivalence---we show the first strictly refines the
other two, which are themselves provably \emph{incomparable}:
logical form neither determines nor is determined by truth
conditions.  We formalize the $N$-operator (joint denial) and
settle TLP~5.52 both ways: quantifiers reduce to $N$-applications
over any finite domain, while over an infinite domain no finite
$N$-application expresses the universal quantifier---the
Geach--Soames critique as a theorem.  We prove full functional
completeness (TLP~5.101) and $N$-generation completeness
(TLP~6)---every proposition is semantically equivalent to one
built from elementaries by iterated $N$---and characterize the
sayable exactly: over finitely many atoms a world-property is
expressible iff invariant under pointwise-$\iff$ agreement of
worlds (and semantic and logical-form equivalence are both
decidable), whereas over infinitely many atoms the totality
property of TLP~1 is invariant yet expressed by no proposition.
The formalization comprises 2,296 lines of \Lean~4 proving 80
verified results with 0 \texttt{sorry} obligations and 1
deliberate axiom (\texttt{axiom silence : True}, encoding
TLP~7).%
...
% Keywords and the 2020 Mathematics Subject Classification are set by
% the class from the \keywords and \subjclass front-matter commands
% (see the preamble); \maketitle follows the abstract in asl.cls.
\maketitle
...
\begin{enumerate}[nosep, label=(\arabic*)]
  ...
  \item A three-way decomposition of all 80 formalized results
...
\noindent
The formalization comprises 2,296 lines of \Lean~4 across six
files.\footnote{%
  ...
  \texttt{TractatusCompleteness.lean} (142 lines, 9),
  \texttt{TractatusExpressibility.lean} (138 lines, 3), and
  \texttt{TractatusDecidability.lean} (148 lines, 3), plus 1
...
\begin{figure}[t]
\centering
% Scale to the (narrower) ASL text column so the three-panel diagram,
% originally drawn for the wider article column, does not overflow.
\resizebox{\textwidth}{!}{%
\begin{tikzpicture}[
  node distance=0.6cm,
  every node/.style={font=\small},
]
  % (a) Free model: full Boolean cube.  Colour is not permitted in
  % ASL/RSL submissions, so all worlds are drawn in black/grey.
  \begin{scope}[xshift=0cm]
    ...
    \foreach \x/\y in {0/0, 2/0, 0/2, 2/2} {
      \fill[black!15, draw=black!60] (\x,\y) circle (4pt);
    }
    \foreach \x/\y in {0.7/0.7, 2.7/0.7, 0.7/2.7, 2.7/2.7} {
      \fill[black!15, draw=black!60] (\x,\y) circle (4pt);
    }
    % Edges of cube
    \draw[black!45, thin] (0,0)--(2,0)--(2,2)--(0,2)--cycle;
    \draw[black!45, thin] (0.7,0.7)--(2.7,0.7)--(2.7,2.7)--(0.7,2.7)--cycle;
    \draw[black!45, thin] (0,0)--(0.7,0.7) (2,0)--(2.7,0.7)
                          (0,2)--(0.7,2.7) (2,2)--(2.7,2.7);
    \node[font=\scriptsize, anchor=north] at (1.35,-0.3)
      {All $2^n$ assignments};
  \end{scope}

  % (b) Weather model: shaded subset.  Valid worlds are filled solid
  % black; excluded worlds are ghosted and crossed out.
  \begin{scope}[xshift=5.0cm]
    \node[font=\small\bfseries, anchor=south] at (1.5,3.3) {(b) Weather model};
    % Draw full cube ghosted
    \foreach \x/\y in {0/0, 2/0, 0/2, 2/2} {
      \fill[black!6, draw=black!20] (\x,\y) circle (4pt);
    }
    \foreach \x/\y in {0.7/0.7, 2.7/0.7, 0.7/2.7, 2.7/2.7} {
      \fill[black!6, draw=black!20] (\x,\y) circle (4pt);
    }
    \draw[black!18, thin] (0,0)--(2,0)--(2,2)--(0,2)--cycle;
    \draw[black!18, thin] (0.7,0.7)--(2.7,0.7)--(2.7,2.7)--(0.7,2.7)--cycle;
    \draw[black!18, thin] (0,0)--(0.7,0.7) (2,0)--(2.7,0.7)
                          (0,2)--(0.7,2.7) (2,2)--(2.7,2.7);
    % Highlight valid worlds (rain->clouds satisfied)
    \foreach \x/\y in {0/0, 0/2, 2/2, 0.7/0.7, 0.7/2.7, 2.7/2.7} {
      \fill[black!70, draw=black] (\x,\y) circle (4pt);
    }
    % Cross out invalid worlds (rain=T, clouds=F)
    \foreach \x/\y in {2/0, 2.7/0.7} {
      \draw[black, thick] (\x-0.1,\y-0.1)--(\x+0.1,\y+0.1);
      \draw[black, thick] (\x-0.1,\y+0.1)--(\x+0.1,\y-0.1);
    }
    \node[font=\scriptsize, anchor=north] at (1.35,-0.3)
  ...
  \begin{scope}[xshift=10.0cm]
    ...
    % Abstract blob
    \fill[black!8, rounded corners=12pt]
      (0,0.5) -- (2.4,0.3) -- (2.4,2.5) -- (0,2.7) -- cycle;
    \draw[black!70, thick, rounded corners=12pt]
      (0,0.5) -- (2.4,0.3) -- (2.4,2.5) -- (0,2.7) -- cycle;
    \foreach \x/\y in {0.4/1.0, 1.2/1.8, 1.8/0.9, 0.8/2.2, 1.9/2.1} {
      \fill[black!45, draw=black!70] (\x,\y) circle (4pt);
    }
    \node[font=\scriptsize, anchor=north] at (1.2,-0.3)
...
\begin{corollary}[Saying requires world-dependence]
...
% This block contains the paper's longest single Lean identifier
% (nontrivial_expressibility_requires_world_dependence), which cannot
% break; \footnotesize keeps it inside the (narrower) ASL text column.
...
totality.  The three results of this subsection carry the same axiom footprint as
...
Over finite atom types the three relations are not merely
separated but \emph{decidable}: the module
\texttt{TractatusDecidability} proves
\texttt{semEq\_iff\_evalBool}---semantic equivalence coincides
with agreement of the computable $\mathsf{Bool}$-valued
evaluators on all $\mathsf{Bool}$ assignments, for an arbitrary
atom type---and derives decision procedures
\texttt{decideSemEq} and \texttt{decideFormEq} once $\Sach$ is a
$\mathsf{Fintype}$.  So the separation of
Theorem~\ref{thm:hierarchy} is checkable by pure computation: at
\texttt{TwoFacts} the witnesses are decided by \texttt{\#eval},
mirroring the evaluator demonstrations of
...
\begin{figure}[t]
\centering
% Scale to the ASL text column so the diagram does not overflow.
\resizebox{\textwidth}{!}{%
\begin{tikzpicture}[every node/.style={font=\small}]
  % Two overlapping regions: formEq (left, hatched) and semEq (right,
  % dotted).  Colour is not permitted in ASL/RSL submissions, so the
  % two regions are distinguished by fill pattern and line style.
  \fill[pattern=north east lines, pattern color=black!35,
        rounded corners=14pt] (-5.0,-2.1) rectangle (1.5,2.1);
  \fill[pattern=dots, pattern color=black!45,
        rounded corners=14pt] (-1.5,-2.1) rectangle (5.0,2.1);
  \draw[black, thick, rounded corners=14pt] (-5.0,-2.1) rectangle (1.5,2.1);
  \draw[black, thick, densely dashed, rounded corners=14pt]
    (-1.5,-2.1) rectangle (5.0,2.1);

  % Region labels
  \node[font=\small\bfseries, fill=white, inner sep=1pt, anchor=north west]
    at (-4.9,2.05) {$\formEq$ (logical form)};
  \node[font=\small\bfseries, fill=white, inner sep=1pt, anchor=north east]
    at (4.9,2.05) {$\semEq$ (truth conditions)};

  % structEq strictly inside the intersection
  \node[draw, very thick, fill=white, rounded corners=4pt,
        minimum width=1.8cm, minimum height=0.7cm] at (0,-1.25) {$\structEq$};

  % Witnesses for each region
  \node[fill=white, inner sep=1pt, align=center, font=\scriptsize] at (-3.3,-0.1)
...
In a companion file
(\texttt{Tractatus\allowbreak Quantifiers.\allowbreak lean}), we
...
% ----------------------------------------------------------------
\subsection{The \texorpdfstring{$N$}{N}-Operator: Finite Success, Infinite Failure}
\label{sec:noperator}
% ----------------------------------------------------------------
...
Our target here differs from the proof-theoretic treatment of
the $N$-operator by Rogers and Wehmeier~\cite{rogers2012}:
their completeness result is about a consequence relation for
a first-order logic with $N$ as sole primitive, whereas
\texttt{nGen\_complete} is about the semantic-equivalence
closure of the elementaries under iterated $N$ within a fixed
development (see Section~\ref{sec:related}).
...
targets.
Closest in spirit to our $N$-operator development is the line
of Wehmeier and Rogers on Tractarian first-order logic.
Wehmeier~\cite{wehmeier2004} develops a Wittgensteinian
predicate logic on the exclusive (``exact'') convention that
identity of object is identity of sign---distinct variables
denote distinct objects, dispensing with a primitive identity
predicate---and gives sound and complete cut-free sequent and
Hilbert calculi for it.  Rogers and Wehmeier~\cite{rogers2012},
published in this journal, take the further step of combining
the exclusive treatment of identity with the $N$-operator as
sole logical primitive, and prove that both Tractarian
proposals can be realized---severally and jointly---in
expressively complete first-order systems with sound and
complete tableau calculi (one for classical logical truth,
others for the Tractarian notion of truth over a fixed
universe of given cardinality).  Their contribution and ours
are complementary rather than competing: they build a
proof-theoretic \emph{logic} in which $N$ is the sole
primitive and settle its completeness on paper, whereas we
mechanize the $N$-operator's expressive reach---its
finite-domain reduction of the quantifiers, its infinite-domain
obstruction, and its iterated-generation completeness---inside
a fixed, machine-checked propositional and quantifier
development (Section~\ref{sec:noperator}).  Our formalization
does not treat identity by the exclusive convention, so their
identity results have no direct analogue here.
...
attempt: a machine-checked axiomatization of Advaita Ved\=anta in
...
\Lean~4: 80 formally verified results across six files that
decompose the \emph{Tractatus} into structural invariants,
model-dependent assumptions, and formal limits of
expressibility.  Two consequences of that decomposition are
worth isolating.  First, the saying/showing distinction becomes
a theorem---nontrivial propositions cannot express
world-independent truths---and the sayable is characterized
exactly, so that the totality of facts of TLP~1, sayable in the
metalanguage, provably lies outside the reach of an object
language whose propositions have finite support.  Second, the
equivalence analysis shows that syntactic identity strictly
refines both logical-form and semantic equivalence, which are
themselves provably incomparable: logical form is not a
way-station between syntax and truth conditions but an
independent dimension of a proposition's identity---a reading of
TLP~4.0141 that only becomes visible under formalization.
...
\texttt{TractatusCompleteness.lean},
\texttt{TractatusExpressibility.lean}, and
\texttt{TractatusDecidability.lean} were produced by Harmonic's
...
\texttt{TractatusExpressibility.lean} and
\texttt{TractatusDecidability.lean}, the support the Aristotle
...
built development; only the headline
statements---\texttt{expressible\_iff\_iff\_invariant},
\texttt{eval\_depends\_only\_on\_atoms},
\texttt{totality\_not\_expressible} (expressibility) and
\texttt{semEq\_iff\_evalBool}, \texttt{decideSemEq},
\texttt{decideFormEq} (decidability)---together with a derived
\texttt{DecidableEq} instance and their finite-support
...
\begin{itemize}[nosep]
  ...
  \item \texttt{proofs/TractatusDecidability.lean} --- 148 lines, 3 declarations
\end{itemize}
...
Tables~\ref{tab:decomposition} and~\ref{tab:decomposition2}
classify every theorem and lemma
...
\centering
\footnotesize
\caption{Complete listing of all 80 theorem/lemma declarations
  plus 1 axiom, classified by the three-way decomposition
  (Part~1 of~2: \emph{TractatusOntology.lean}).
...
\centering
\footnotesize
\caption{Theorem listing, Part~2 of~2: the remaining five files and
  the axiom (class codes as in Table~\ref{tab:decomposition}).}
\label{tab:decomposition2}
\begin{tabular}{@{}rlllc@{}}
\toprule
\# & \textbf{Lean name} & \textbf{TLP} & \textbf{File} & \textbf{Cl.} \\
\midrule
...
\midrule
\multicolumn{5}{l}{\emph{TractatusDecidability.lean} (3 declarations)} \\
\midrule
78 & \texttt{semEq\_iff\_evalBool}                      & 4.0141  & Dec. & E \\
79 & \texttt{decideSemEq}                               & 4.0141  & Dec. & E \\
80 & \texttt{decideFormEq}                              & 4.0141  & Dec. & E \\
\midrule
...
\bibitem{rogers2012}
B.~Rogers and K.~F. Wehmeier,
``Tractarian first-order logic: Identity and the $N$-operator,''
\emph{The Review of Symbolic Logic}, vol.~5, no.~4,
pp.~538--573, 2012.
\newline doi:~\href{https://doi.org/10.1017/S1755020312000032}{\texttt{10.1017/S1755020312000032}}.

\bibitem{wehmeier2004}
K.~F. Wehmeier,
``Wittgensteinian predicate logic,''
\emph{Notre Dame Journal of Formal Logic}, vol.~45, no.~1,
pp.~1--11, 2004.
\newline doi:~\href{https://doi.org/10.1305/ndjfl/1094155275}{\texttt{10.1305/ndjfl/1094155275}}.
