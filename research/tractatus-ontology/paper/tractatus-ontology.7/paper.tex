...
The formalization comprises 1,933 lines of \Lean~4, proves 71
...
\begin{enumerate}[nosep, label=(\arabic*)]
  ...
  \item A three-way decomposition of all 71 formalized results
...
\noindent
The formalization comprises 1,933 lines of \Lean~4 across four
files.\footnote{%
  \texttt{TractatusOntology.lean} (1,345 lines, 45 theorem/lemma
  declarations), \texttt{TractatusQuantifiers.lean} (284 lines, 9),
  \texttt{TractatusNOperator.lean} (162 lines, 8), and
  \texttt{TractatusCompleteness.lean} (142 lines, 9), plus 1
...
(cf.\ the form-series of TLP~6, which we do not formalize; see
Section~\ref{sec:limitations}).
...
The $N$-operator itself is now formalized
(Section~\ref{sec:noperator}), but Wittgenstein's full
claim---that all propositions arise by \emph{successive}
applications of $N$ along a form-series
(TLP~6, $[\bar{p}, \bar{\xi}, N(\bar{\xi})]$)---is not.  Our
results concern a single $N$-application over a list of
arguments; iterated application and the form-series notation
remain open.
...
First, formalizing iterated $N$-application along a form-series
(TLP~6) would extend the single-application results of
Section~\ref{sec:noperator} to Wittgenstein's general
propositional form.
...
\Lean~4.  The formalization proves 71 formally verified results
across four files, decomposes the
...
The proofs in \texttt{TractatusNOperator.lean} and
\texttt{TractatusCompleteness.lean} were produced by Harmonic's
...
\texttt{Quot.sound} only.  All definitions, theorem statements,
and the remaining proofs are ours.
...
\begin{itemize}[nosep]
  ...
  \item \texttt{proofs/TractatusNOperator.lean} --- 162 lines, 8 declarations
...
\centering
\footnotesize
\caption{Complete listing of all 71 theorem/lemma declarations
...
\midrule
\multicolumn{5}{l}{\emph{TractatusNOperator.lean} (8 declarations)} \\
\midrule
...
\midrule
63 & \texttt{disj\_evalBool}                            & 5.101   & Comp. & E \\
64 & \texttt{literal\_evalBool}                         & 5.101   & Comp. & E \\
65 & \texttt{truePropOf\_evalBool}                      & 5.101   & Comp. & E \\
66 & \texttt{falsePropOf\_evalBool}                     & 5.101   & Comp. & E \\
67 & \texttt{conjFold\_evalBool}                        & 5.101   & Comp. & E \\
68 & \texttt{minterm\_evalBool}                         & 5.101   & Comp. & E \\
69 & \texttt{bigDisj\_evalBool}                         & 5.101   & Comp. & E \\
70 & \texttt{dnf\_evalBool}                             & 5.101   & Comp. & E \\
71 & \texttt{functional\_completeness}                  & 5.101   & Comp. & I \\
\midrule
