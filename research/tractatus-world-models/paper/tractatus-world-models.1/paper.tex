...
\author{Robb Walters}
\date{July 2026 \quad\textbar\quad Draft v1}
...
space. All results are formalized in Lean~4 and checked by its kernel: 63
...
\cite{wittgenstein1929}. Hacker's verdict is representative: Wittgenstein's
first philosophy ``collapsed over its inability to solve one problem ---
colour exclusion'' \cite{hacker1986}.
...
\item\label{c:horn} \textbf{An exact independence boundary.} For Horn
  ...
  truth-value profile is realizable iff it satisfies every clause. This gives
...
equivalence, and exclusion developments comprise 63 declarations
($\sim$1{,}000 lines) with no \lean{sorry}s and an axiom footprint of
\lean{propext}, \lean{Classical.choice}, and \lean{Quot.sound} only. Theorem
...
\begin{figure}[t]
  ...
  exclusion tiers are refinement-incomparable: no Horn model shares a
...
\TLP{3.42}'s dictum that ``a proposition can determine only one place in
logical space, but the whole of logical space must already be given by it''
reads naturally as the observation that the ambient lattice is fixed by $S$
alone, however the modeling choice restricts the realized region.
...
\lean{horn\_realizable\_iff}.)
\end{theorem}

Read contrapositively, this says precisely which inferences ``from the
...
the refinement-theoretic phrasing --- the two tiers are incomparable regions
of the model spectrum, not merely distinct syntactic classes --- and the
mechanization.
\end{remark}
...
no \lean{sorry}s remain. We follow the methodological line of the
mechanization of G\"odel's ontological argument \cite{benzmuller2014}: where
a philosophical dispute turns on \emph{what follows from what}, mechanization
relocates the dispute to its honest residue --- whether the formal
explication is faithful. For the present paper that residue is concentrated
...
the exclusion-tier module were developed with an AI assistant and verified by
the Lean kernel; the spectrum, Horn, and equivalence modules originate in the
companion project \cite{companion2026}, with machine-verified proofs
throughout (see Acknowledgments).
...
acknowledged there. The exclusion-tier module (statements and proofs) was
...
All declarations reside in the \lean{tractatus} repository
(\lean{proofs/TractatusOntology\{Horn,Equiv,Spectrum,Exclusion\}.lean});
the build is warning-free under Lean~4.26 with no \lean{sorry}s.
...
Thm~\ref{thm:horn-boundary} & \lean{horn\_realizable\_iff} & Horn \\
...
Lem~\ref{lem:top} & \lean{hornModel\_allTrue\_realizable}, \lean{exclusionModel\_allTrue\_not\_realizable} & Exclusion \\
...
Wittgenstein, Ludwig. \emph{Tractatus Logico-Philosophicus}. Translated by
D.~F. Pears and B.~F. McGuinness. London: Routledge \& Kegan Paul, 1961.
First published 1921 as ``Logisch-philosophische Abhandlung''.

\bibitem{wittgenstein1929}
Wittgenstein, Ludwig. ``Some Remarks on Logical Form.'' \emph{Proceedings of
the Aristotelian Society}, Supplementary Volume 9 (1929): 162--171.

\bibitem{ramsey1923}
Ramsey, Frank P. ``Critical Notice of L.~Wittgenstein's \emph{Tractatus
Logico-Philosophicus}.'' \emph{Mind} 32, no.~128 (1923): 465--478.

\bibitem{hacker1986}
Hacker, P.~M.~S. \emph{Insight and Illusion: Themes in the Philosophy of
Wittgenstein}. Revised edition. Oxford: Clarendon Press, 1986.

\bibitem{moss2012}
Moss, Sarah. ``Solving the Color Incompatibility Problem.'' \emph{Journal of
Philosophical Logic} 41, no.~5 (2012): 841--851.
doi:10.1007/s10992-011-9193-3.

\bibitem{button2017}
Button, Tim. ``Exclusion Problems and the Cardinality of Logical Space.''
\emph{Journal of Philosophical Logic} 46, no.~6 (2017): 611--623.
doi:10.1007/s10992-016-9412-z.

\bibitem{skyrms1981}
Skyrms, Brian. ``Tractarian Nominalism.'' \emph{Philosophical Studies} 40,
no.~2 (1981): 199--206.

\bibitem{armstrong1989}
Armstrong, D.~M. \emph{A Combinatorial Theory of Possibility}. Cambridge:
Cambridge University Press, 1989.

\bibitem{spinney2022}
Spinney, Oliver Thomas. ``Logical Form and Logical Space in Wittgenstein's
\emph{Tractatus}.'' \emph{Synthese} 200, no.~1 (2022), article 31.
doi:10.1007/s11229-022-03470-y.

\bibitem{lokhorst1988}
Lokhorst, Gert-Jan C. ``Ontology, Semantics and Philosophy of Mind in
Wittgenstein's \emph{Tractatus}: A Formal Reconstruction.'' \emph{Erkenntnis}
29, no.~1 (1988): 35--75.

\bibitem{weiss2017}
Weiss, Max. ``Logic in the Tractatus.'' \emph{The Review of Symbolic Logic}
10, no.~1 (2017): 1--50. doi:10.1017/S1755020316000472.

\bibitem{companion2026}
Walters, Robb. ``Saying, Showing, and Structure: A Machine-Checked
Reconstruction of the \emph{Tractatus} Ontology.'' Manuscript under review,
2026. Proof artifact: the \lean{tractatus} Lean~4 repository.

\bibitem{evans2014}
Evans, Richard Prideaux, and Martin Berger. ``Cathoristic Logic: A Modal
Logic of Incompatible Propositions.'' arXiv:1411.7158 (2014).

\bibitem{mckinsey1943}
McKinsey, J.~C.~C. ``The Decision Problem for Some Classes of Sentences
Without Quantifiers.'' \emph{The Journal of Symbolic Logic} 8, no.~3 (1943):
61--76.

\bibitem{horn1951}
Horn, Alfred. ``On Sentences Which Are True of Direct Unions of Algebras.''
\emph{The Journal of Symbolic Logic} 16, no.~1 (1951): 14--21.

\bibitem{benzmuller2014}
Benzm\"uller, Christoph, and Bruno Woltzenlogel Paleo. ``Automating G\"odel's
Ontological Proof of God's Existence with Higher-Order Automated Theorem
Provers.'' In \emph{Proceedings of ECAI 2014}, 93--98. IOS Press, 2014.

\bibitem{moura2021}
de Moura, Leonardo, and Sebastian Ullrich. ``The Lean 4 Theorem Prover and
Programming Language.'' In \emph{Automated Deduction -- CADE 28}, LNCS 12699,
